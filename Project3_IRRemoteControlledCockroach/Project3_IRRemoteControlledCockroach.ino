...
void loop() {
    if(nec_ok) {                                     // If a good NEC message is received
        ...
        if(command == 0x10){  // Remote v1 = 0x10  | Remote v2 = 0xE0
            oled.println("Turn Left");
            motorMoveControl(0, 0, 20);
            motorMoveControl(1, 1, 20);    
        }

        else if(command == 0x18){ // Remote v1 = 0x18  | Remote v2 = 0x02
            oled.println("Move Forward");
            motorMoveControl(0, 1, 20);
            motorMoveControl(1, 1, 20);            
        }

        else if(command == 0x5A){ // Remote v1 = 0x5A  | Remote v2 = 0x90
            oled.println("Turn Right");
            motorMoveControl(0, 1, 20);
            motorMoveControl(1, 0, 20);            
        }
            
        else if(command == 0x4A){ // Remote v1 = 0x4A  | Remote v2 = 0x98
            oled.println("Move Backward");
            motorMoveControl(0, 0, 20);
            motorMoveControl(1, 0, 20);            
        }
                  
        else if(command == 0x38){ // Remote v1 = 0x38  | Remote v2 = 0xA8
            oled.println("Stop");    
